...
export default function Admin_Campaign_Chat({ campaign, onClose }: ChatCampaignProps) {
    ...
    useEffect(() => {
        ...
        socket.onmessage = (event) => {
            try {
                ...
                // FIX: accept messages that have content OR file_url
                if (data.content !== undefined || data.file_url) {
                    const normalized = {
                        ...data,
                        id: data.id ?? data.message_id ?? Date.now(),  // ← ADD THIS
                    };
    ...
    // Step 1: user picks file -> show preview in input bar (no upload yet)
    ...
    // Step 2: user clicks Send -> upload file -> WS broadcast adds it to chat
    ...
    const renderMessageContent = (msg: Message, isAdmin: boolean) => {
            ...
            return (
                <a href={msg.file_url} target="_blank" rel="noreferrer" download={msg.file_name}
                    ...
                    <div style={{
                        ...
                        background: isAdmin ? "rgba(255,255,255,0.2)" : "#F1F5F9",
                    ...
                    <div style={{
                        ...
                        background: isAdmin ? "rgba(255,255,255,0.2)" : "#E2E8F0",
            ...
            <span>
                {parts.map((part, i) =>
                    /^@LI[A-Z0-9]+$/.test(part) ? (
                        <span key={i} style={{
                            fontWeight: 700,
                            background: isAdmin ? "rgba(255,255,255,0.25)" : "rgba(0,87,184,0.10)",
                            color: isAdmin ? "#fff" : "#0057B8",
    ...
    const availableTags = [...new Set(
        messages.filter(m => m.tagged_line_item).map(m => m.tagged_line_item!)
    )];

    const displayedMessages = filterTag
        ? messages.filter(m => m.tagged_line_item === filterTag)
        : messages;
    ...
    return (
        <>
            ...
            <div style={{
                ...
                {/* Line Item Filter Bar — admin only */}
                    ...
                    <div style={{
                        ...
                        <button
                            ...
                            style={{
                                ...
                                border: filterTag === null ? "1.5px solid #0057B8" : "1px solid #E2E8F0",
                                background: filterTag === null ? "#EFF6FF" : "#fff",
                                color: filterTag === null ? "#0057B8" : "#94A3B8",
                                cursor: "pointer"
                            }}>All</button>
                        {availableTags.map(tag => (
                            <button key={tag} onClick={() => setFilterTag(tag === filterTag ? null : tag)}
                                style={{
                                    fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 10,
                                    border: filterTag === tag ? "1.5px solid #0057B8" : "1px solid #E2E8F0",
                                    background: filterTag === tag ? "#EFF6FF" : "#fff",
                                    color: filterTag === tag ? "#0057B8" : "#64748B",
                        ...
                        return (
                            <div key={msg.id} style={{
                                ...
                                <div style={{
                                    ...
                                    boxShadow: isAdmin ? "0 2px 8px rgba(0,87,184,0.25)" : "0 1px 4px rgba(0,0,0,0.08)",
