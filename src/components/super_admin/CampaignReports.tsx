    ...
    interface InternalEdits {
        impressions: string;      // CPM = impressions booked, CPC = clicks booked
        start_date: string;
        end_date: string;
        advertiser_id: string;    // ← ADD (default from campaign, editable per line item)
        target_cpm: string;       // ← ADD (CPM only)
        target_ctr: string;       // ctr
        target_cpc: string;       // ← ADD (CPC only)
        booked_budget: string;    // ← ADD (CPC only)
        sitelist: string;
    }
    ...
    interface PreviewModalProps {
        ...
        saving: boolean;
        onClose: () => void;
        // Client tab — original read-only data
        clientData: any[][];
        // Internal tab — editable fields
    ...
    function PreviewModal({
        ...
        // Deduplicated correct fields per report type
        ...
        return (
            <div style={{
                ...
                <div style={{
                    background: C.white, borderRadius: 16, width: "80vw", maxWidth: 960,
                    ...
                    <div style={{
                        ...
                        <button onClick={onClose} style={{
                            ...
                            color: C.slate500, display: "flex", alignItems: "center", padding: 4, borderRadius: 6,
                        }}>
                        ...
                        {[
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    color: activeTab === tab.key
                                        ? (tab.key === 'internal' ? C.green : C.blue)
                                        : C.slate500,
                                    background: 'none', border: 'none',
                                    borderBottom: activeTab === tab.key
                                        ? `2px solid ${tab.key === 'internal' ? C.green : C.blue}`
                            ...
                            >
                                ...
                                <span style={{
                                    ...
                                    background: tab.key === 'internal' ? C.greenLight : C.blueLight,
                                    color: tab.key === 'internal' ? C.green : C.blue,
                                    border: `1px solid ${tab.key === 'internal' ? C.greenMid : C.blueMid}`,
                                    textTransform: 'uppercase',
                                }}>
                                    {tab.desc}
                                </span>
                    ...
                    {activeTab === 'client' && (
                        <div style={{
                            display: 'flex', gap: 0, borderBottom: `1px solid ${C.border}`,
                            padding: '0 24px', background: '#FAFBFF', flexShrink: 0,
                        }}>
                            {sheetNames.map(name => (
                                <button
                                    key={name}
                                    ...
                                    <tbody>
                                        {clientData.map((row, rIdx) => (
                                            <tr key={rIdx}>
                                                {rIdx === 0 ? (
                                                    <td colSpan={2} style={{
                                                        background: "#1F4E79", padding: "8px 12px",
                                                    ...
                                                    row.map((cell: any, cIdx: number) => (
                                                        <td key={cIdx} style={{
                                                            ...
                                                            padding: "6px 10px",
                                                            background: cIdx === 0 ? "#D9E1F2" : (rIdx % 2 === 0 ? "#F8FAFC" : "#fff"),
                                                            fontSize: 12,
                                                            fontWeight: cIdx === 0 ? 600 : 400,
                                                            color: cIdx === 0 ? "#1e293b" : "#64748b",
                                                        }}>
                                                            {cell ?? ''}
                                                        </td>
                                                    ))
                        ...
                        {activeTab === 'internal' && (
                            <>
                                ...
                                <div style={{
                                    ...
                                    📋 Currently editing: <span style={{ fontFamily: 'monospace', fontWeight: 700 }}>{activeSheet}</span>
                    ...
                    <div style={{
                        ...
                        <div style={{ fontSize: 11.5, color: C.slate500 }}>
                            {activeTab === 'client'
                            ...
                            {activeTab === 'internal' && (
                                <Button
                                    ...
                                    style={{
                                        ...
                                        fontSize: 13, background: C.green, borderColor: C.green,
                                    }}
                                >
    ...
    export default function CampaignReports() {
        ...
        const [internalEdits, setInternalEdits] = useState<InternalEdits>({
            ...
            advertiser_id: '',
            target_cpm: '', target_ctr: '', target_cpc: '',
            booked_budget: '', sitelist: '',
        });
        ...
        // ── Preview Excel ──────────────────────────────────────────────────────
        // Update handlePreview to also fetch existing internal edits:
        const handlePreview = async (record: CampaignRow) => {
            ...
            try {
                // Re-generate for fresh client data
                ...
                // Fetch client Excel for display
                ...
                // Fetch existing internal edits for first sheet's line item
                ...
                if (editsRes.ok) {
                    ...
                    const firstLineItemId = firstSheet; // sheet name = line_item_id
                    const existing = editsData[firstLineItemId] || {};
                    setInternalEdits({
        ...
        // When sheet changes, load that line item's internal edits:
        ...
        // New handleSaveEdits — sends internal edits only, line_item_id from activeSheet:
        const handleSaveEdits = async () => {
            setSaving(true);
            try {
                const payload = {
                    report_type: previewReportType,
                    line_item_id: activeSheet, // sheet name = line_item_id
                    ...internalEdits,
                };
                ...
                {showPreview && (
                    previewLoading ? (
                        ...
                        <PreviewModal
                            ...
                            onSheetChange={handleSheetChange}   // ← use new handler
