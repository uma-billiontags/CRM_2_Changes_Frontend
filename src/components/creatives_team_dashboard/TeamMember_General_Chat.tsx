...
export default function TeamMember_General_Chat() {
    const { user } = useAuth();          // expects user.id and user.username/role from your auth context
    ...
    useEffect(() => {
        ...
        const socket = new WebSocket(`${WS_BASE_URL}/ws/internal-chat/${userId}/`);
        socketRef.current = socket;
        socket.onopen = () => console.log(`internal chat connected: ${userId}`);
        ...
        socket.onmessage = (event) => {
            try {
                ...
                console.error("Internal chat WS message error", e);
            }
        };

        socket.onclose = () => console.log("Internal chat WS closed");
        return () => { socket.close(); };
    ...
    const uploadStagedFile = async () => {
        ...
        formData.append("sender_type", "member");
        try {
            const res = await fetch(`${BASE_URL}/send_internal_chat_file/${userId}/`, { method: "POST", body: formData });
            if (!res.ok) console.error("Internal chat file upload failed");
        } catch (err) {
            console.error("Internal chat file send error", err);
        } finally {
            setUploading(false);
            clearStaged();
        }
    ...
    const handleSend = () => {
        ...
        if (!text || !socketRef.current || !userId) return;
        ...
        if (msg.message_type === "file" && msg.file_url) {
            return (
                <a href={msg.file_url} target="_blank" rel="noreferrer" download={msg.file_name}
                    ...
                    <div style={{ width: 40, height: 40, borderRadius: 8, flexShrink: 0, background: isMember ? "rgba(255,255,255,0.2)" : "#F1F5F9", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>
    ...
    if (!userId) return null;  // don't render for unauthenticated/unknown users
                ...
                <>
                    ...
                    <div style={{
                        ...
                        <div style={{ background: C.blue, padding: "14px 16px", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
                            <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#1d4ed8", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 800, color: "#fff", border: "2px solid rgba(255,255,255,0.35)" }}>A</div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ fontSize: 13, fontWeight: 700, color: "#fff" }}>Admin</div>
                                <div style={{ fontSize: 11, color: "rgba(255,255,255,0.75)" }}>{userName}</div>
                            </div>
                        ...
                        <div style={{ flex: 1, overflowY: "auto", padding: "16px 14px", display: "flex", flexDirection: "column", gap: 10, background: "#F8FAFC" }}>
                            ...
                            {messages.map(msg => {
                                ...
                                return (
                                    <div key={msg.id} style={{ display: "flex", flexDirection: "column", alignItems: isMember ? "flex-end" : "flex-start", marginBottom: 12 }}>
                                        <div style={{
                                            maxWidth: isFileMsg ? "85%" : "78%",
                                            padding: (msg.message_type === "image" || msg.message_type === "video") ? "6px" : "9px 13px",
                                            borderRadius: isMember ? "14px 14px 4px 14px" : "14px 14px 14px 4px",
                                            background: isMember ? C.blue : "#fff",
                                            color: isMember ? "#fff" : "#1E293B", fontSize: 13,
                                            boxShadow: isMember ? "0 2px 8px rgba(37,99,235,0.25)" : "0 1px 4px rgba(0,0,0,0.08)",
                                            border: isMember ? "none" : "1px solid #E2E8F0", overflow: "hidden",
                                        }}>
